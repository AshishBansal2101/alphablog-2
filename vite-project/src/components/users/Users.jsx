...
const Users = () => {
  ...
  const current_user = { id: 12, admin: true };
  const followed = (user) => {
    return false;
  };
  return (
              ...
              <div>
                <div className="col-xl-5 col-sm-7 mb-5 mt-5 mx-auto shadow-lg p-3 mb-5  rounded">
                  <div className="bg-white rounded shadow-sm py-5 px-4">
                    ...
                    <p className="card-text">{user.articles.count} article </p>
                    <Link to="/viewprofile" className="btn btn-success mr-2">
                      View Profile
                    </Link>
                    {logged_in && (
                      <>
                        ...
                        {user.id != current_user.id && followed(user) && (
                          <Link to="/unfollow" className="btn btn-info mr-2">
                            Following
                          </Link>
                        )}
                        {user.id != current_user.id && (
                          <Link to="/Follow" className="btn btn-info mr-2">
                            Follow
                          </Link>
                        )}
