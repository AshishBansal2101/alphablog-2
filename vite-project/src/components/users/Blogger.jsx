import React from "react";
import { useState, useEffect, useContext } from "react";
import { AuthContext } from "../../AuthContext";
import { Link } from "react-router-dom";
import axios from "axios";

const Blogger = ({ props }) => {
  const { user } = useContext(AuthContext);
  // const ftrue=props?.followers?.include(user)
  const ftrue = props?.followers.some((o) => o.id === user.id);
  const [following, setFollowing] = useState(ftrue);
  // console.log("blogger", props)


  const follow = (e) => {
    setFollowing(true);
    e?.preventDefault();
    axios
      .put(
        `http://localhost:3000/follow`,
        {
          c_user: {
            id: user.id,
          },
          follow: {
            id: props.id,
          },
        },
        {
          headers: {
            Authorization: localStorage.getItem("token"),
          },
        }
      )
      .then((response) => {
        console.log("follow", response.data);
        if (response.data == "successfull") alert("followed");
      })
      .catch((err) => {
        console.log(err);
      });
  };

  const unfollow = (e) => {
    setFollowing(false);
    e?.preventDefault();
    axios
      .put(
        `http://localhost:3000/unfollow`,
        {
          c_user: {
            id: user.id,
          },
          unfollow: {
            id: props.id,
          },
        },
        {
          headers: {
            Authorization: localStorage.getItem("token"),
          },
        }
      )
      .then((response) => {
        console.log("unfollow", response.data);
        if (response.data == "successfull") alert("unfollowed");
      })
      .catch((err) => {
        console.log(err);
      });



  };

  return (
    <div>
      <div>
        <div className="col-xl-5 col-sm-7 mb-5 mt-5 mx-auto shadow-lg p-3 mb-5  rounded">
          <div className="bg-white rounded shadow-sm py-5 px-4">
            <img
              src="data:image/png;base64,"
              alt=""
              width="100"
              className="img-fluid rounded-circle mb-3 img-thumbnail shadow-sm"
            />
            <h5 className="mb-0">
              <Link to="/user">{props?.username}</Link>
            </h5>
            <span className="small text-uppercase text-muted">
              {props?.followers?.length} Followers
            </span>
            <p className="card-text">{props?.articles?.length} article </p>
            <Link
              to={`/viewprofile/${props?.id}`}
              className="btn btn-success mr-2"
            >
              View Profile
            </Link>

            <>
              {props?.id == user?.id && (
                <Link to="/editprofile" className="btn btn-success mr-2">
                  Edit Profile
                </Link>
              )}
              {user?.admin && (
                <Link to="/deleteuser" className="btn btn-outline-danger mr-2">
                  Delete User
                </Link>
              )}
              {props?.id != user?.id && following && (
                <Link onClick={unfollow} className="btn btn-info mr-2">
                  Following
                </Link>
              )}
              {props?.user?.id != user?.id && !following && (
                <Link onClick={follow} className="btn btn-info mr-2">
                  Follow
                </Link>
              )}

              <div className="card-footer text-muted mt-3">
                <small>Joined {props?.created_at} ago</small>
              </div>
            </>
          </div>{" "}
        </div>
      </div>
    </div>
  );
};

export default Blogger;
